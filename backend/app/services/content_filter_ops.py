...
foreach my $bank (qw(MYNETS MYUSERS ORIGINATING PICKUP)) {{
  next unless defined $policy_bank{{$bank}};
  $policy_bank{{$bank}}{{'bypass_spam_checks_maps'}} = [0];
  $policy_bank{{$bank}}{{'final_spam_destiny'}} = D_QUARANTINE;
}}
...
def _diagnostics(filters: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    amavis_hook_loaded = (
        ...
        and "package Amavis::Custom" in amavis_content
    )
...
def _apply_filters(filters: list[dict[str, Any]]) -> list[str]:
    ...
    custom_content = _build_amavis_custom_filters(filters)
    custom_path = amavis_custom_filters_path()
