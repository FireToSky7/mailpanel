...
def _build_amavis_config_preamble(filters: list[dict[str, Any]]) -> str:
    """Perl config directives for amavisd.conf (not inside Amavis::Custom)."""
...
def _build_amavis_hook_block(filters: list[dict[str, Any]], custom_path: Path) -> str:
    preamble = _build_amavis_config_preamble(filters).rstrip()
    do_line = f"do '{custom_path.as_posix()}';"
    parts = ["# Generated by MailPanel. Do not edit manually."]
    if preamble:
        parts.append(preamble)
    parts.append(do_line)
    return "\n".join(parts)
...
def _diagnostics(filters: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        and "@av_scanners = ();" in amavis_content,
