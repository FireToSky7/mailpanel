...
sub _quarantine_match {{
  my ($conn, $msginfo, $field) = @_;
  ...
  my $blocked = 0;
  $msginfo->add_contents_category(&main::CC_BANNED, 0);
  for my $r (@{{$msginfo->per_recip_data}}) {{
    next if $r->recip_done;
    $r->add_contents_category(&main::CC_BANNED, 0);
    eval {{
      Amavis::do_quarantine($conn, $msginfo, $r, [$quar_to], $method);
    }};
    if ($@) {{
      Amavis::Util::do_log(0, "MAILPANEL: do_quarantine failed for <%s>: %s", $r->recip_addr, $@);
      next;
    }}
    $r->recip_done(1);
  ...
  Amavis::Util::do_log(0, "MAILPANEL: quarantined (%s) count=%d <%s>", $field, $blocked, $msginfo->sender || '?')
    if $blocked;
}}
...
sub new {{
  ...
  my @subj = _subject_candidates($msginfo);
  Amavis::Util::do_log(0, "MAILPANEL: new() from=<%s> subj=<%s>", $msginfo->sender || '?', $subj[0] || '');
...
sub checks {{
  ...
  _quarantine_match($conn, $msginfo, $field);
  $self->{{quarantined}} = 1;
}}
...
# RelayedInternal (MYUSERS) otherwise bypasses spam checks and ignores CC_SPAM.
foreach my $bank (qw(MYUSERS MYNETS RELAY ORIGINATING)) {
  next unless defined $policy_bank{$bank};
